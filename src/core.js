...
export function buildIssues({
  ...
  for (const record of records || []) {
    const studentId = normalizeId(record.studentId);
    const baseline = byStudent.get(`${normalizeId(camp?.id)}:${studentId}`) || byStudent.get(studentId) || (includeAll ? {
      studentId,
    ...
    const parsedCompletionRate = record.completionRate === null || record.completionRate === undefined || record.completionRate === ""
      ? null
      : Number(record.completionRate);
    const completionRate = Number.isFinite(parsedCompletionRate) ? parsedCompletionRate : null;
    const unfinishedInClassHomework = incompleteHomework(record.inClassHomework);
    const unfinished = incompleteHomework(record.homework);
    const unfinishedExtensions = incompleteHomework(record.extensions);
    const issueTypes = [];
    let transferState = "";

    if (!attendanceAt || completionRate === 0) issueTypes.push("absence");
    if (unfinishedInClassHomework.length > 0) issueTypes.push("inclass");
    if (unfinished.length > 0) issueTypes.push("homework");
    if (unfinishedExtensions.length > 0) issueTypes.push("extension");
    ...
    if (!includeAll && !issueTypes.length) continue;
    issues.push({
      ...
      attendanceAt,
      completionRate,
      inClassHomework: normalizeHomeworkItems(record.inClassHomework),
      incompleteInClassHomework: unfinishedInClassHomework,
...
export function filterIssues(issues, filters = {}) {
  ...
  return (issues || []).filter((issue) => {
    ...
    const searchable = normalizeText(`${issue.studentName || ""} ${issue.studentId || ""}`).toLowerCase();
    if (queryTerms.length && !queryTerms.every((term) => fuzzyContains(searchable, term))) return false;
    return true;
  });
}
...
function compareStudentIdsDescending(leftId, rightId) {
  if (!leftId || !rightId) {
  ...
  return rightId.localeCompare(leftId, "zh-CN", { numeric: true, sensitivity: "base" });
}

export function sortIssuesByStudentId(issues) {
  return sortIssuesNewestFirst(issues)
    .map((issue, index) => ({ issue, index, studentId: normalizeId(issue.studentId) }))
    .sort((left, right) => {
      const comparison = compareStudentIdsDescending(left.studentId, right.studentId);
