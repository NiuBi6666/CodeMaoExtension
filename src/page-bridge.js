(() => {
  ...
  function lessonHintFromTemplate() {
    const template = templates.get("teachSearch");
    if (!template) return { ids: [], courseIds: [], name: "" };
    const ids = new Set();
    ...
    const inspect = (value) => {
      ...
      for (const [key, child] of Object.entries(value)) {
        ...
        if (/^(lessonid|lessonids|camplessonid|courselessonid|lbkcourseid)$/.test(normalizedKey)) addId(child);
        if (normalizedKey === "courseid") {
          for (const item of Array.isArray(child) ? child : [child]) {
            const normalized = String(item ?? "").trim();
            if (normalized) courseIds.add(normalized);
          }
        }
        if (!name && /^(lessonname|coursename|lbkcoursename)$/.test(normalizedKey)) name = String(child ?? "").trim();
        if (child && typeof child === "object") inspect(child);
      }
    };
    try {
      const url = new URL(template.url, location.href);
      for (const key of ["lessonId", "campLessonId", "courseLessonId", "lbkCourseId"]) addId(url.searchParams.get(key));
      const courseId = String(url.searchParams.get("courseId") ?? "").trim();
      if (courseId) courseIds.add(courseId);
    } catch {
      // The JSON request body usually contains the same lesson identifier.
    }
    inspect(parseJson(template.body));
    return { ids: [...ids], courseIds: [...courseIds], name };
  }
  ...
  function patchPayload(value, params, parentKey = "") {
    ...
    for (const [key, child] of Object.entries(value)) {
      ...
      if (normalizedKey === "classid") {
        if (params.allClasses) next = null;
        else if (params.classId != null) next = params.classId;
      }
      ...
      if (normalizedKey === "classuserteacherwechatquicklyoperate" && params.quicklyOperate !== undefined) {
        next = params.quicklyOperate;
      }
  ...
  function buildReplayRequest(kind, params) {
    ...
    if (typeof body === "string" && body.trim().startsWith("{")) {
      ...
      if (parsed) {
        const patched = patchPayload(parsed, params);
        if (kind === "teachSearch" && params.quicklyOperate !== undefined) {
          if (params.quicklyOperate == null) delete patched.classUserTeacherWechatQuicklyOperate;
          else patched.classUserTeacherWechatQuicklyOperate = params.quicklyOperate;
        }
      ...
      if (message.type === "STATE_REQUEST") {
        post({
          ...
          result: {
            ...
            lessonHint: lessonHintFromTemplate()
          }
        });
        return;
      }
